...
test_ratio = 0.20 # percentage of datapoints that are for testing. The rest will be for test
export = False # to export classifier model with filename or not
download_confusion = False # download confusion matrix or not
# ------------------------------------------------------------------------------------------------------------------------------------------------------------------------


train_df = pd.read_pickle("text.pkl") # insert path to df with columns of "lower" (uncleaned) and "lemmatized" (column with clean text)
...
if balance_method == "delete":
    plot_name = f"{name_text}_balanced-del_{desc}"
elif balance_method == "duplicate":
    plot_name = f"{name_text}_balanced-dup_{desc}"
else:
    plot_name = f"{name_text}_unbalanced_{desc}"
...
def display_balance(df):
    ...
    sns.countplot(x = df["label"]).set(title = "Label Balance Tweets")
    plt.show()
...
if download_confusion:
    fig.savefig(f"{plot_name}.png", dpi = 100)
